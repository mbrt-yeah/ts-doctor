...
const handleFunctionSchema: TFileSchemaHandler = async function(
    ...
    const finalCardinality = (parameters.itemSchema.cardinality === "infinity") ? parameters.apiDoc.functions.length : parameters.itemSchema.cardinality;

    let opError: Error | undefined;

    for (let i = 0; i < finalCardinality; i++) {
        const fileNamer = new FileAndFolderNamer({
            entityDoc: parameters.apiDoc.functions[i],
        ...
        const page: IPage<IFunctionDoc> = new Page<IFunctionDoc>({
            content: parameters.apiDoc.functions[i],
            contentSelector: `apiDoc.functions[${i}]`,
